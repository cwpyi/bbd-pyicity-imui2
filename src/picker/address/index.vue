<template>
    <div class="address-picker">
        <transition name="fade">
            <div class="_picker_mask" @click="cancel" v-if="addressShow"></div>
        </transition>
                    ...
                    <div class="content">
                        <div class="content-item" :class="{w50:!hasTown}" v-picker.province="{set:set}" ref="province">
                            <div class="picker-item">
                                <div class="_item-text" v-for="(val,index) of address" :class="{color:index===pIndex}"
                                     :key="val.name">{{val.name}}</div>
                            </div>
                        </div>
                        <div class="content-item" :class="{w50:!hasTown}" v-picker.city="{set:set}" ref="city">
                            <div class="picker-item">
                                <div class="_item-text" v-for="(val,index) of city" :class="{color:index===cIndex}"
                                     :key="index">{{val.name?val.name:val}}</div>
                            </div>
                        </div>
                        <div class="content-item" :class="{w0:!hasTown}" v-picker.town="{set:set}" ref="town">
                            <div class="picker-item">
                                <div class="_item-text" v-for="(val,index) of town" :class="{color:index===tIndex}" :key="val">
                                    {{val}}</div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            hasTown() {
                return this.showT && this.address[this.pIndex].cityList[this.cIndex].areaList;
            }
        },
        methods: {
            ok() {
                let province = this.address[this.pIndex];
                let city = province.cityList[this.cIndex];
                const data = {province: province.name, city: city.name};
                if(this.showT){
                    data.town = city.areaList?city.areaList[this.tIndex]:'';
                }
                //每次点击确定事件，将地址选择器重置到初始状态
                this.pIndex = 0;
                this.cIndex = 0;
                this.onok && this.onok(data)
            },
            cancel() {
            ...
            picker: {
                bind(el, binding) {
                    ...
                    el.addEventListener('touchend', (e) => {
                        ...
                        index = Math.floor(Math.abs(currentY));//记录当前位移在数组中的索引,必须取整，否则会出现浮点数
                        const name = binding.modifiers.province ? 'province' : binding.modifiers.city ? 'city'
                            : binding.modifiers.town ? 'town' : '';
